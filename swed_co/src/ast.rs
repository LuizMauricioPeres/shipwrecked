//! Abstract Syntax Tree for Harbour source — shared between `swed` and `swed_nm`.
//!
//! Nodes are cheap to clone (Strings, Vecs — no Rc/Arc).
//! Every node carries a `SrcSpan` for diagnostics.
//! Representation is Harbour-semantic, not Rust-semantic; the codegen maps it.

/// Source span — byte offset range inside a `.prg` file.
pub type SrcSpan = std::ops::Range<usize>;

// ---------------------------------------------------------------------------
// Top-level program
// ---------------------------------------------------------------------------

/// A compiled `.prg` file: one or more procedures / functions / classes.
#[derive(Debug, Clone)]
pub struct Program {
...
pub struct AtSayStmt {
    ...
    pub col:   Expr,
    pub expr:  Expr,
    /// Harbour COLOR clause — stored for future Ratatui mapping.
...
pub struct AssignStmt {
    /// LHS: simple ident, array element (a[i]), or OOP field (obj:field)
    pub target: Expr,
    pub value: Expr,
}
...
pub enum ExprKind {
    ...
    Integer(i64),
    Float(f64),
    /// `"string"`, `'string'`, and `[string]` all collapse here.
    String(String),
    /// `{ e1, e2, … }` — array literal
    ArrayLit(Vec<Expr>),
