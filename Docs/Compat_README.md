...
> Funções de compatibilidade com Clipper e variantes — geralmente evite usar diretamente.
...
**Parâmetros:**
  ...
  - `<lFlag>` — optional <lFlag> or <cOnOff> (not case sensitive) .T. or "ON" to enable the century setting (4-digit years) .F. or "OFF" to disable the century setting (2-digit years)
...
**Parâmetros:**
  ...
  - `<nFunctionKey>` — <nFunctionKey> is a number in the range 1..40 that represent the function key to be assigned.
  - `<cString>` — <cString> is a character string to set. If <cString> is not specified, the function key is going to be set to NIL releasing by that any previous __SetFunction() or SetKey() for that function.
...
**Parâmetros:**
  ...
  - `<cFile>` — <cFile> is either "PRINTER" (note the uppercase) in which console output is SET to PRINTER, or a name of a text file with a default ".txt" extension, that is used to redirect console output.
...
**Parâmetros:**
  ...
  - `<nHandle>` — <nHandle>  Dos file handle
  - `<nOffset>` — <nOffset>  Offset of the first byte of the region to be locked.
  - `<nBytes>` — <nBytes>   Number of bytes to be locked.
  - `<nType>` — <nType>    The type (read or write) of lock requested.
...
**Parâmetros:**
  ...
  - `<nHandle>` — <nHandle>  Dos file handle set>  Offset of the first byte of the region to be locked.
  - `<nBytes>` — <nBytes>   Number of bytes to be locked.
...
**Parâmetros:**
  ...
  - `<lNewError>` — <lNewError> Is a logical Expression.
